(define (domain Rover2)
       ...
       (:types
              rover waypoint store camera mode lander objective battery-number
       ) ; Anadimos battery-number
       ...
       (:predicates
              ...
              (battery-predecessor ?b - battery-number ?c - battery-number) ; Anadimos esta linea
              (has-battery ?r - rover ?b - battery-number) ; Anadimos esta linea
       )

       (:action navigate
              :parameters (?x - rover ?b - battery-number ?c - battery-number ?y - waypoint ?z - waypoint)
              :precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y)
                     (visible ?y ?z)
                     (has-battery ?x ?b) ; Verificamos que coincida el numero de bateria
                     (battery-predecessor ?c ?b) ; Verificamos que tenga bateria suficiente
              )
              :effect (and (not (at ?x ?y)) (at ?x ?z) (not (has-battery ?x ?b)) (has-battery ?x ?c)
              ); Cambiamos la bateria
       )

       ; cargar bateria
       (:action charge
              :parameters (?x - rover ?b - battery-number ?c - battery-number ?l - lander ?w - waypoint)
              :precondition (and (has-battery ?x ?b) (battery-predecessor ?b ?c) (at_lander ?l ?w) (at ?x ?w))
              :effect (and (not (has-battery ?x ?b)) (has-battery ?x ?c))
       )
